...
func _physics_process(delta):
	...
	Head.rotation_degrees.x = clamp(Head.rotation_degrees.x , -35 ,35)
	if Head.rotation_degrees.x >= 35:
		Head.rotation_degrees.x = lerp(Head.rotation_degrees.x , 30 , 5* delta)
